...
- **Painel Administrativo Completo:** Uma área restrita (`/admin`) protegida por autenticação, onde os professores gerenciam seus Níveis, Perguntas e Alunos.
- **Poderoso Dashboard Analítico (Novo!):** O sistema agora conta com um dashboard avançado, alimentado por banco de dados relacional e `vue-chartjs`. O professor tem à disposição gráficos e métricas ricas de desempenho em duas frentes: **Métricas da Turma** (agregando resultados globais) e **Métricas do Aluno** (desempenho individual e curva de aprendizado). O sistema mapeia dificuldades da turma, separa resultados por tipo de pergunta, tempo médio de resolução e identifica padrões (ex: se o aluno está "chutando" as questões).
- **Emissão de Certificados Dinâmicos (Novo!):** Ao finalizar os desafios, os alunos desbloqueiam um certificado de conclusão (Diploma) colorido e em alta qualidade, totalmente personalizável e que pode ser salvo na hora como imagem (`html2canvas`) para celebrar a conquista!
